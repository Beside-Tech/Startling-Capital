...
function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  const initials = user?.name ? user.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2) : "LP";
    ...
    <div className="flex flex-col h-full bg-sidebar border-r border-sidebar-border">
      ...
      <div className="px-3 py-4 border-t border-sidebar-border space-y-2">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          <div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center text-amber-700 font-bold text-xs shrink-0">{initials}</div>
...
export function LPLayout({ children }: { children: React.ReactNode }) {
