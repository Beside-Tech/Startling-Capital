...
function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  const initials = user?.name ? user.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2) : "IC";
    ...
    <div className="flex flex-col h-full bg-sidebar border-r border-sidebar-border">
      <div className="flex items-center justify-between px-4 py-4 border-b border-sidebar-border">
        <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => navigate("/")}>
          ...
          <div>
            ...
            <div className="text-[10px] text-muted-foreground leading-none">Investment Committee</div>
          </div>
        </div>
        {onClose && (
      ...
      <div className="px-3 py-4 border-t border-sidebar-border space-y-2">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <button onClick={logout} className="text-muted-foreground hover:text-destructive transition-colors" title="Sign out">
...
export function ICLayout({ children }: { children: React.ReactNode }) {
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-border bg-background shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-display font-bold text-sm">Startling Capital IC</span>
          </div>
          <div className="w-5" />
        </div>
